...
\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=0.4\textwidth]{images/precision-recall.jpg}}
...
La mAP deriva da una metrica più elementare, l'Average Precision (AP), che si calcola separatamente per ogni classe. Variando la soglia di confidenza, precision e recall cambiano,
ordinando le predizioni dalla più alla meno confidente e accumulandole una alla volta si ottiene la curva precision-recall della classe, e l'AP ne misura l'area sottesa.

In formula, indicando con $p(r)$ la precision in corrispondenza di una recall $r$:
$$ AP = \int_{0}^{1} p_{\mathrm{interp}}(r)\, dr \qquad \text{con} \qquad p_{\mathrm{interp}}(r) = \max_{r' \geq r} p(r') $$
dove $p_{\mathrm{interp}}$ è la curva resa monotona decrescente, così da eliminare le piccole oscillazioni della precision.

La mAP è semplicemente la media delle AP calcolate sulle $N$ classi del dataset:
$$ mAP = \frac{1}{N} \sum_{c=1}^{N} AP_c $$
L'AP descrive quindi le prestazioni del modello su una singola classe, mentre la mAP le riassume in un unico valore per l'intero modello.

Negli articoli scientifici relativi al rilevamento di oggetti, è possibile imbattersi in abbreviazioni come  \(mAP_{50}\) o \(mAP_{95}\).
In breve, questa notazione indica la soglia IoU utilizzata per stabilire se una predizione è corretta durante il calcolo delle AP.
La \(mAP_{50}\) usa una sola soglia pari a 0,5, mentre la notazione estesa \(mAP_{50:95}\) indica la media delle mAP ottenute con dieci soglie comprese tra 0,5 e 0,95, a passi di 0,05:
$$ mAP_{50:95} = \frac{1}{10} \sum_{t \in \{0.50,\, 0.55,\, \dots,\, 0.95\}} mAP_{t} $$
...
\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=0.75\textwidth]{images/IoU.jpg}}
    \cite{IoU}
...
\subsection{Determinismo delle metriche}

Sebbene le metriche appena descritte restituiscano valori numerici precisi, l'intero ecosistema di valutazione nasconde una natura intrinsecamente stocastica.
Risulta quindi fondamentale distinguere il piano puramente matematico della misurazione da quello pratico che genera le predizioni.

Il calcolo delle metriche è rigorosamente deterministico: indici come IoU o mAP, a parità di predizioni e ground truth, restituiscono sempre l'identico risultato.
L'incertezza risiede a monte, nelle complesse dinamiche di addestramento. In questa fase, l'inizializzazione dei pesi, l'applicazione del dropout,
la data augmentation e la gestione asincrona dei thread sulle GPU introducono componenti casuali ineliminabili.
Addestrare più volte lo stesso modello produce quindi reti con pesi finali disallineati; per questo motivo, variazioni minime nelle metriche vanno interpretate come naturali fluttuazioni statistiche,
e non come prove di effettivi miglioramenti architetturali. \cite{tf_nondeterminism}

Sorprendentemente, una quota di variabilità sopravvive anche in fase di inferenza, a pesi ormai congelati. Poiché l'aritmetica in virgola mobile non gode della proprietà associativa,
l'ordine di esecuzione delle somme altera lievemente il risultato numerico finale.
Questo ordine dipende da fattori puramente operativi, come la dimensione del batch size, l'hardware o i dettagli implementativi delle librerie sottostanti.
La stessa documentazione ufficiale di PyTorch avverte che la totale riproducibilità tra versioni software e piattaforme diverse o tra CPU e GPU non è garantita di default,
a meno di non forzare un determinismo che penalizzerebbe fortemente i tempi di calcolo. \cite{batch_invariance} \cite{pytorch_randomness}

Infine, va considerato che i punteggi di confidenza dei moderni detector risultano tipicamente mal calibrati e tendono a un'eccessiva sicurezza,
non riflettendo la reale probabilità statistica di correttezza della predizione.
Per questi motivi, i valori restituiti dalle metriche non vanno intesi come garanzie assolute sul comportamento del modello in produzione, bensì come stime statistiche empiriche,
strettamente ancorate a una singola esecuzione dell'addestramento e al limitato campione di immagini utilizzato. \cite{detector_calibration}
...
\begin{figure}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth]{images/training YOLO26n/training-stats.png}}
    \caption{statistiche}
...
\begin{figure}[h!]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1\textwidth]{images/training YOLO26n/confusion_matrix_normalized.png}}
